			...
			switch(msg_type)
			{
			  ...
			  case MQTT_MSG_TYPE_PUBACK:
				if(mqtt_state.pending_msg_type == MQTT_MSG_TYPE_PUBLISH && mqtt_state.pending_msg_id == msg_id){
				  os_printf("MQTT: Publish successful\r\n");
			  ...
			  case MQTT_MSG_TYPE_PUBCOMP:
				if(mqtt_state.pending_msg_type == MQTT_MSG_TYPE_PUBLISH && mqtt_state.pending_msg_id == msg_id){
				  os_printf("MQTT: Public successful\r\n");
...
MQTT_Sub()
{
	os_printf("MQTT: Send subscribe, topic:");
	os_printf(mqtt_topic[topic_idx]);
	os_printf("\r\n");
...
MQTT_Task(os_event_t *events)
{
	...
	case WIFI_CONNECTED:
		...
		if(UTILS_StrToIP(sysCfg.mqtt_host, &pCon->proto.tcp->remote_ip)) {
			os_printf("CONNECT TO IP:");
			os_printf(sysCfg.mqtt_host);
			os_printf("\r\n");
			espconn_connect(pCon);
			connState = TCP_CONNECTING;
		}
		else {
			os_printf("CONNECT TO HOST:");
			os_printf(sysCfg.mqtt_host);
			os_printf("\r\n");
	...
	case MQTT_DATA:
		if(mqtt_state.outbound_message != NULL){
			...
			if(mqtt_state.pending_msg_type == MQTT_MSG_TYPE_PUBLISH && mqtt_state.pending_msg_id == 0)
				os_printf("MQTT: Publish message is done!\r\n");
			break;
		}
		break;
	}
