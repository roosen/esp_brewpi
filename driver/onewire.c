...
// more certain timing.
//
static void ICACHE_FLASH_ATTR write_bit(int v)
{
...
// more certain timing.
//
static int ICACHE_FLASH_ATTR read_bit(void)
{
	int r;
	GPIO_OUTPUT_SET(ONEWIRE_PIN, 0);
	os_delay_us(3);
...
int ICACHE_FLASH_ATTR onewire_search(uint8_t *newAddr)
{
	...
	if (!LastDeviceFlag) {
		...
		onewire_write(ONEWIRE_SEARCHROM, 0);
...
// Do a ROM select
//
void ICACHE_FLASH_ATTR onewire_select(const uint8_t *rom)
{
	uint8_t i;
	onewire_write(ONEWIRE_MATCHROM, 0); // Choose ROM
	for (i = 0; i < 8; i++)
		onewire_write(rom[i], 0);
}

//
// Do a ROM onewire_skip
//
void ICACHE_FLASH_ATTR onewire_skip()
{
    onewire_write(ONEWIRE_SKIPROM, 0); // Skip ROM
}

void ICACHE_FLASH_ATTR onewire_search_reset()
{
	int i;
...
uint8_t ICACHE_FLASH_ATTR onewire_reset(void)
{
	int r;
	uint8_t retries = 125;
	GPIO_DIS_OUTPUT(ONEWIRE_PIN);
...
// other mishap.
//
void ICACHE_FLASH_ATTR onewire_write(uint8_t v, int power)
{
	uint8_t bitMask;
...
// Read a byte
//
uint8_t ICACHE_FLASH_ATTR onewire_read(void)
{
	uint8_t bitMask;
	uint8_t r = 0;
	for (bitMask = 0x01; bitMask; bitMask <<= 1) {
